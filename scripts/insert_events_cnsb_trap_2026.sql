...
BEGIN
    -- 1. Get Club ID (CNSB)
    -- Intenta por siglas o nombre
    SELECT id INTO v_club_id FROM clubes WHERE siglas ILIKE '%CNSB%' OR nombre ILIKE '%CNSB%' OR nombre ILIKE '%Nacional de Silueta%' LIMIT 1;
    ...
    IF v_club_id IS NULL THEN
        RAISE EXCEPTION 'Club "CNSB" not found';
    END IF;
    ...
    RAISE NOTICE 'Evento de Trap Americano (CNSB) insertado correctamente.';
END $$;
